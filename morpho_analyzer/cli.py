...
import morpho_analyzer
from morpho_analyzer import opencorpora_dict, test_suite_generator
from morpho_analyzer.vendor.docopt import docopt
from morpho_analyzer.utils import download_bz2, get_mem_usage

logger = logging.getLogger('morpho_analyzer')
...
def compile_dict(in_filename, out_path=None, overwrite=False, prediction_options=None):
    ...
    opencorpora_dict.convert_to_morpho_analyzer(
...
def show_dict_mem_usage(dict_path=None, verbose=False):
    ...
    morph = morpho_analyzer.MorphAnalyzer(dict_path)
...
Morpho_analyzer is a morphological analyzer / inflection engine for Russian language.
"""
__doc__ ="""
Usage::

    m_analyzer dict compile <XML_FILE> [--out <PATH>] [--force] [--verbose] [--max_forms_per_class <NUM>] [--min_ending_freq <NUM>] [--min_paradigm_popularity <NUM>]
    m_analyzer dict download_xml <OUT_FILE> [--verbose]
    m_analyzer dict mem_usage [--dict <PATH>] [--verbose]
    m_analyzer dict make_test_suite <XML_FILE> <OUT_FILE> [--limit <NUM>] [--verbose]
    m_analyzer dict meta [--dict <PATH>]
    m_analyzer _parse <IN_FILE> <OUT_FILE> [--dict <PATH>] [--verbose]
    m_analyzer -h | --help
    m_analyzer --version
...
def main():
    ...
    """
    args = docopt(DOC, version=morpho_analyzer.__version__)
